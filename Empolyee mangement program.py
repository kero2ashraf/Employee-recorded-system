...
def add():
    if ref_var.get()==0 or name_var.get()=='' or gender_var.get()=='' or age_var.get()==0 or email_var.get()=='' or phone_var.get()=='' or desi_var.get()=='':
...
def save():
    ...
    f1=open(r'C:\Users\dell\OneDrive\Desktop\New folder (2)\ref'+str(ref_var.get())+'.txt','w')
    f1.write(data)
    f1.close()
...
def print():
    ...
    data = textarea.get(1.0,END)
    f = r'C:\Users\dell\OneDrive\Desktop\New folder (2)\ref'+str(ref_var.get())+'.txt'
...
combo_des['value']=('Hr','Accountant','IT','Sales','Finance',"others")
...
lbl_age=Label(F1,text='Age',font=('times new rommon',20,'bold'),fg='black',bg=bg_color)
lbl_age.grid(row=6,column=0,padx=30,pady=10)
txt_age=Entry(F1,font=('times new rommon',18,'bold'),relief=RIDGE,bd=7,textvariable=age_var)
txt_age.grid(row=6,column=1,pady=10,sticky='w')
